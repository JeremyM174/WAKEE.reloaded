...
if start_button:
    ...
    if not cap.isOpened():
        ...
        cap.release()
else: # Reseting all parameters when camera is turned off, before asking for user feedback
    with col2:
        image_display.empty()
        stats_display.empty()
        ...
        frames_evaluation_list = []

        try:
            for captured_frames in os.listdir("temp"):
                if captured_frames.lower().endswith(valid_extensions):
                    frames_evaluation_list.append(captured_frames)

            frame_to_evaluate = random.choice(frames_evaluation_list)

            with rating_display.form("experience_rating_form"):
                st.markdown("### Help us improve WAKEE! Rate this frame: \n(0=missing, 3=strongly represented)")
                image_display.image(f"temp/{frame_to_evaluate}")

                rating_boredom = st.selectbox(
                    "Boredom",
                    options=[0, 1, 2, 3],
                    index=None,
                ...
                rating_engagement = st.selectbox(
                    "Engagement",
                    options=[0, 1, 2, 3],
                    index=None,
                    placeholder="Select a rating",
                ...
                rating_frustration = st.selectbox(
                    "Frustration",
                    options=[0, 1, 2, 3],
                    index=None,
                    placeholder="Select a rating",
                ...
                if submitted:
                    ratings = [rating_boredom, rating_engagement, rating_confusion, rating_frustration]

                    if any(r is None for r in ratings):
                        st.error("Please provide a rating for all four fields.")

                    else:
                        feedback_row = pd.DataFrame(
                            [{
                                "image_path": os.path.basename(frame_to_evaluate),
                                "boredom": rating_boredom,
                                "engagement": rating_engagement,
                                "confusion": rating_confusion,
                                "frustration": rating_frustration,
                            }]
                        )

                        feedback_row.to_csv(f"temp/{os.path.basename(frame_to_evaluate)}.csv", index=False)
                        
                        image_display.empty()
                        rating_display.empty()
                        stats_display.markdown("Thank you, your feedback has been saved!")
        except Exception:
            stats_display.markdown("Ready when you are!")
